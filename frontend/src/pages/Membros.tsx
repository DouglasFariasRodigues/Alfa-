...
import { useMembros, useDeleteMembro, useMembrosEstatisticas } from "@/hooks/useMembros";
...
export default function Membros() {
  ...
  // Buscar membros da API apenas se tiver permissão
  const { data: membros = [], isLoading: isLoadingMembros, error: errorMembros } = useMembros({ 
    search: searchTerm || undefined,
    status: statusFilter || undefined 
  }, { enabled: canManageMembers });
  
  // Buscar estatísticas (sempre disponível)
  const { data: estatisticas, isLoading: isLoadingEstatisticas, error: errorEstatisticas } = useMembrosEstatisticas();
  
  const deleteMembroMutation = useDeleteMembro();
  
  // Usar estatísticas da API se disponível, senão calcular dos membros
  const isLoading = canManageMembers ? isLoadingMembros : isLoadingEstatisticas;
  const error = canManageMembers ? errorMembros : errorEstatisticas;
  ...
  // Se tiver permissão, usar dados dos membros; senão, usar estatísticas da API
  const totalMembros = canManageMembers 
    ? membros.length 
    : (estatisticas?.total || 0);
  const membrosAtivos = canManageMembers 
    ? membros.filter(m => m.status === 'ativo').length 
    : (estatisticas?.ativos || 0);
  const membrosInativos = canManageMembers 
    ? membros.filter(m => m.status === 'inativo').length 
    : (estatisticas?.inativos || 0);
  const novosEsteMes = canManageMembers 
    ? (() => {
  ...
  return (
    <div className="p-6 space-y-6">
        ...
        <div>
          ...
          <p className="text-muted-foreground">
            {canManageMembers 
              ...
              : "Visualize estatísticas dos membros da comunidade"}
          </p>
        </div>
        {canManageMembers && (
      ...
      {/* Members List - Apenas para quem tem permissão */}
      {canManageMembers && (
        <Card className="shadow-card">
          <CardHeader>
      ...
      {!canManageMembers && (
        <Card className="shadow-card">
          <CardHeader>
            <CardTitle>Informações sobre Membros</CardTitle>
            <CardDescription>
              Você pode visualizar apenas as estatísticas gerais dos membros
            </CardDescription>
          </CardHeader>
          <CardContent>
            <div className="text-center py-8">
              <p className="text-muted-foreground">
                Para visualizar detalhes dos membros, é necessário ter permissão de gerenciamento.
              </p>
            </div>
          </CardContent>
        </Card>
